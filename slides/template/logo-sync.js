(() => {
  const updateLogo = () => {
    ...
    const hasSectionImage = isSection && slide.classList.contains("has-image");
    const sectionImage = hasSectionImage
      ? getComputedStyle(slide).getPropertyValue("--section-image").trim()
      : "none";

    reveal.classList.toggle("cl-logo-white", isSection);
    reveal.classList.toggle("cl-section-active", isSection);
    document.body.classList.toggle("cl-section-active", isSection);
